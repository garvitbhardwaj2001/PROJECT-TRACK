...
review = input('What do you feel about the movie?')
review = [review]
vectorizer = pickle.load(open('TF-IDF.sav', 'rb'))
review = vectorizer.transform(review)

classifier = pickle.load(open('TASK-1.sav','rb'))
pred = classifier.predict(review)

print('Your review will be stored as',pred[0],'.')
...
for i in range(0,11):
    ...
    y_pred.append(classifier2.predict(X_test))
    pred.append(classifier2.predict(tweet))
